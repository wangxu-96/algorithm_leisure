 ...
 * 存在一个比它小的数字：（1）。 对于 nums[4]=3 存在三个比它小的数字：（1，2 和 2）。 示例 2：
 * 
...
public class SmallerNumbersThanCurrent {
    ...
    public static void main(String[] args) {
        SmallerNumbersThanCurrent smaller = new SmallerNumbersThanCurrent();
        int[] result = smaller.smallerNumbersThanCurrent(new int[] { 8, 1, 2, 2, 3 });
        for (int i : result) {
